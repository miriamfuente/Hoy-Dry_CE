fun.intensity <- function(hottest.month, pr.obs, tmax.obs, pr.daily, tmax.daily) {
    ...
    source("functions/fun.define.intensity.R")
    # pr.daily = pr.transformed (only data in the sesason (3 months))
    # pr50 <- climatology(pr.daily, clim.fun = list(FUN = "quantile", probs=0.5, na.rm = TRUE))
    # pr40 <- climatology(pr.daily, clim.fun = list(FUN = "quantile", probs=0.4, na.rm = TRUE))
    # pr30 <- climatology(pr.daily, clim.fun = list(FUN = "quantile", probs=0.3, na.rm = TRUE))
    # pr20 <- climatology(pr.daily, clim.fun = list(FUN = "quantile", probs=0.2, na.rm = TRUE))
    # pr10 <- climatology(pr.daily, clim.fun = list(FUN = "quantile", probs=0.1, na.rm = TRUE))
    ...
    t90 <- climatology(tmax.daily, clim.fun = list(FUN = "quantile", probs=0.9, na.rm = TRUE))
